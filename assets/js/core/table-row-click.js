...
export function isInteractiveElement(target) {
    ...
    return Boolean(target.closest(
        ...
        '[data-no-row-click], [data-custom-select], .custom-select-trigger, .custom-select-dropdown'
    ));
}
 ...
 * @param {Function} options.onRowClick - Callback nhận vào (id, row, event)
 */
export function enableTableRowClick({ tbody, rowSelector, idAttribute = "data-course-id", onRowClick }) {
    const container = typeof tbody === "string" ? document.querySelector(tbody) : tbody;
    ...
    container.addEventListener("click", (e) => {
        ...
        // 4. Lấy ID
        const id = row.getAttribute(idAttribute) || row.dataset.courseId || row.dataset.id;
        if (id && typeof onRowClick === "function") {
            onRowClick(id, row, e);
        }
    });
