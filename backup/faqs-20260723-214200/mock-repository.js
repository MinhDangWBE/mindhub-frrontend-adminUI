...
function getDB() {
  ...
  try {
    ...
    if (!Array.isArray(db.banners)) { db.banners = MOCK_DB.banners || []; mutated = true; }
...
export function calculateBannerEffectiveStatus(banner, now = new Date()) {
  if (!banner) return { effective_status: "inactive", is_currently_visible: false };
  
  const status = banner.status || "inactive";
  if (status === "inactive") {
    return { effective_status: "inactive", is_currently_visible: false };
  }

  const startAt = banner.start_at ? new Date(banner.start_at) : null;
  const endAt = banner.end_at ? new Date(banner.end_at) : null;
  const hasStart = startAt && !isNaN(startAt.getTime());
  const hasEnd = endAt && !isNaN(endAt.getTime());

  if (hasStart && startAt > now) {
    return { effective_status: "scheduled", is_currently_visible: false };
  }

  if (hasEnd && endAt < now) {
    return { effective_status: "expired", is_currently_visible: false };
  }

  return { effective_status: "active", is_currently_visible: true };
}

export function populateBanner(banner) {
  if (!banner) return null;
  const { effective_status, is_currently_visible } = calculateBannerEffectiveStatus(banner);
  return {
    id: Number(banner.id),
    title: String(banner.title || ""),
    image_url: String(banner.image_url || ""),
    target_url: banner.target_url ? String(banner.target_url) : null,
    position: String(banner.position || "home_top"),
...
export function getBanners(params = {}) {
  const db = getDB();
  const rawBanners = db.banners || [];
  
  // Non-deleted banners
  const nonDeleted = rawBanners.filter(b => !b.deleted_at).map(populateBanner);
  
  // Summary across ALL non-deleted banners
  const summary = {
    total_banners: nonDeleted.length,
    active_count: nonDeleted.filter(b => b.effective_status === "active").length,
    inactive_count: nonDeleted.filter(b => b.effective_status === "inactive").length,
    scheduled_count: nonDeleted.filter(b => b.effective_status === "scheduled").length,
    expired_count: nonDeleted.filter(b => b.effective_status === "expired").length
  };
  ...
  if (params.search) {
    ...
    filtered = filtered.filter(b =>
      ...
      (b.image_url && b.image_url.toLowerCase().includes(q)) ||
      (b.target_url && b.target_url.toLowerCase().includes(q)) ||
      (b.position && b.position.toLowerCase().includes(q))
    );
  }

  // Filter position
  if (params.position && params.position !== "all" && params.position !== "") {
    filtered = filtered.filter(b => b.position === params.position);
  }
  ...
  // Filter effective view_mode
  if (params.view_mode && params.view_mode !== "all" && params.view_mode !== "") {
    filtered = filtered.filter(b => b.effective_status === params.view_mode);
  }
  ...
  const perPage = Math.max(1, parseInt(params.per_page) || 20);
...
export function getBannerById(id) {
  const db = getDB();
  const banner = (db.banners || []).find(b => Number(b.id) === Number(id));
  if (!banner || banner.deleted_at) return null;
  return populateBanner(banner);
}

export function createBanner(data) {
  const db = getDB();
  db.banners = db.banners || [];
  const maxId = db.banners.reduce((max, item) => Math.max(max, Number(item.id) || 0), 10000);
  const newId = maxId + 1;
  const now = new Date().toISOString();
  ...
  const newBanner = {
    ...
    target_url: data.target_url ? String(data.target_url).trim() : null,
  ...
  saveDB(db);
  return populateBanner(newBanner);
}

export function updateBanner(id, patchData) {
  const db = getDB();
  db.banners = db.banners || [];
  const index = db.banners.findIndex(b => Number(b.id) === Number(id));
  if (index === -1 || db.banners[index].deleted_at) return null;
  ...
  if (patchData.sort_order !== undefined) updated.sort_order = Number(patchData.sort_order);
  if (patchData.target_url === "") updated.target_url = null;

  db.banners[index] = updated;
  saveDB(db);
  return populateBanner(updated);
}
...
export function deleteBanner(id) {
  ...
  db.banners = db.banners || [];
  const index = db.banners.findIndex(b => Number(b.id) === Number(id));
  if (index === -1 || db.banners[index].deleted_at) return false;

  db.banners[index].deleted_at = new Date().toISOString();
  db.banners[index].updated_at = new Date().toISOString();
  saveDB(db);
  return true;
}
